...
use cc_token_usage::output::text::{
    ...
    render_validation, render_wrapped,
};
...
fn main() -> Result<()> {
    ...
    match command {
        ...
        Command::Validate { id, failures_only } => {
            ...
            if want_json {
                eprintln!(
                    "Note: JSON output is not yet supported for validate. Showing text instead."
                );
            }
        ...
        Command::Wrapped { year } => {
            ...
            if want_html && !want_text {
                eprintln!(
                    "Note: HTML output is not yet supported for wrapped. Showing text instead."
                );
                println!("{}", render_wrapped(&result));
            }
        }
